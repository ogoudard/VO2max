...
void GadgetBle::setCurrentWifiSsid(std::string ssid) {
    ...
    if (_wifiSsidChar != NULL) {
        _wifiSsidChar->setValue(String(_wifiSsidSetting.c_str()));
    }
}
...
void GadgetBle::onWrite(BLECharacteristic* characteristic) {
    if (characteristic->getUUID().toString().compareTo(LOGGER_INTERVAL_UUID) ==
        0) {
        ...
        _sampleBufferSize = 0;
        _sampleCntChar->setValue(_sampleBufferSize);
    } else if (characteristic->getUUID().toString().compareTo(
                   WIFI_SSID_CHAR_UUID) == 0) {
        _wifiSsidSetting = std::string(characteristic->getValue().c_str());
    } else if (characteristic->getUUID().toString().compareTo(
                   WIFI_PWD_CHAR_UUID) == 0) {
        std::string wifiPwd = std::string(characteristic->getValue().c_str());
...
void GadgetBle::_bleInit() {
    ...
    std::string macAddress =  std::string(BLEDevice::getAddress().toString().c_str());
    ...
    if (_onWifiSettingsChanged != NULL) {
        _wifiSsidChar = bleGadgetSettingsService->createCharacteristic(
            WIFI_SSID_CHAR_UUID, BLECharacteristic::PROPERTY_READ |
                                     BLECharacteristic::PROPERTY_WRITE);
        _wifiSsidChar->setValue(_wifiSsidSetting.c_str());
    ...
    // Modification suggested by chatgpt
    //_bleAdvertising->setMinPreferred(0x0);
    //_bleAdvertising->setMaxPreferred(0x0);
    //bleServer->getAdvertising()->setMinInterval(0x50); // 80 ms
    //bleServer->getAdvertising()->setMaxInterval(0x100); // 256 ms
    //_bleAdvertising->setScanResponse(false);
...
void GadgetBle::_updateAdvertising() {
    ...
    scanResponse.setManufacturerData(String(manufData.c_str()));
    _bleAdvertising->setScanResponseData(scanResponse);

    BLEAdvertisementData advert;
    advert.setName(GADGET_NAME);
    advert.setManufacturerData(String(manufData.c_str()));
