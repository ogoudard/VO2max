...
{
    WriteCommandWithArgument(SCD30_SET_TEMP_OFFSET, offset);
}
...
bool SCD30_IsAvailable(void)
{
    ...
    WriteReadCommand(SCD30_GET_DATA_READY, readBuffer, sizeof(readBuffer));

    return (readBuffer[1] != 0x00);
}
...
void SCD30_SetAutoSelfCalibration(bool enable)
{
    if (enable)
    {
        WriteCommandWithArgument(SCD30_AUTOMATIC_SELF_CALIBRATION, 1); // Activate continuous ASC
    }
    else
    {
        WriteCommandWithArgument(SCD30_AUTOMATIC_SELF_CALIBRATION, 0); // Deactivate continuous ASC
    }
}

void SCD30_SetMeasurementInterval(uint16_t interval)
{
    WriteCommandWithArgument(SCD30_SET_MEASUREMENT_INTERVAL, interval);
}

void SCD30_StartPeriodicMeasurment(void)
{
    WriteCommandWithArgument(SCD30_CONTINUOUS_MEASUREMENT, 0x0000);
}

void SCD30_StartPeriodicMeasurmentWithPressureCompensation(uint16_t pressure)
{
    WriteCommandWithArgument(SCD30_CONTINUOUS_MEASUREMENT, pressure);
}
...
bool SCD30_GetMeasures(float *co2, float *temperature, float *humidity)
{
    ...
    WriteReadCommand(SCD30_READ_MEASUREMENT, readBuffer, sizeof(readBuffer));
